function request(args...; kwargs...)
    @static if VersionNumber(VERSION.major, VERSION.minor) ≥ v"1.12"
        isdefined(Main, :VSCodeServer) &&
            ...
            "A working way to install package(s) into a shared env would be to execute the ShareAdd macro or function from Terminal " * 
            """or run your script from Terminal, or execute VSCode command "Julia: Run File in New Process" once. """ *
            "After package installation, you can return to normal use of VSCode. Sorry for the inconvenience. See ShareAdd documentation for details.") maxlog = 1
